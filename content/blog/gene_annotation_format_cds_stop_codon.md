---
title: "从一个 CDS 边界问题看基因组注释格式的差异：GTF、GFF3、BED 与 genePred 的一些记录"
summary: " 从一个 CDS 边界问题看基因组注释格式的差异"
date: 2026-06-03
draft: false
...
> 这是一篇项目记录性质的整理。起点是一个很具体的问题：在多物种基因组数据分析中，同样叫作 “GTF 参考注释文件”，为什么有些文件的 `CDS` 坐标包含终止密码子，而有些文件却把 `stop_codon` 单独列出来，并且 `CDS` 本身不包含终止密码子？继续追下去后发现，这并不是某一个数据库偶然写错，而是 GTF/GFF 历史演化、不同数据库数据模型、格式转换工具以及社区实践共同造成的结果。
